...
def do_strands(s,sequence,double_type=True,rotation=None,rise_per_bp=None):
    ...
    if double_type == False:
        ...
        while not added:
            ...
            if not added:
                print('Overlap detected, trying to place the strand elsewhere', file=sys.stderr)
        if oN == s._N: # if the number of nucleotides has not changed, adding strand has failed.
        ...
        while not added:
            ...
            if not added:
                print('Overlap detected, trying to place the strands elsewhere', file=sys.stderr)
        if oN == s._N: # if the number of nucleotides has not changed, adding strand has failed.
            print('Adding strand ', sequence, 'failed', s._N)
...
def process_sequence_file(fname,box):
    ...
    for line in fin.readlines():
        ...
        if(vals[0] == 'DOUBLE' ):
            ...
            print('Adding duplex 3\'-', seq,'-5\'', file=sys.stderr)
            do_strands(s,track,True)
        else:
            seq = vals[0].strip()
            track = [base.base_to_number[x] for x in seq]
            print('Adding sequence 3\'-', seq,'-5\'', file=sys.stderr)
