...
def stackarray (lines, nbases):
    ret = [False for i in range(nbases - 1)]
    for line in lines:
        words = line.split ()
        if len(words) == 10:
            i = int (words[0])
...
total = [0 for i in range(mysystem._N - 1)]
lengths = [0 for i in range(mysystem._N)]
...
while mysystem != False:
    ...
    print(mysystem._time, a, file=sys.stderr)
    print(mysystem._time, lengths, file=sys.stderr)
