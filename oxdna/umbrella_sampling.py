...
class UmbrellaSampling:
    ...
    def __init__ (self, dir='dir', exe='./oxDNA', input='input',
        ...
        if os.path.isdir (dir):
            ...
            print(UmbrellaSampling.initError, dir, "does not exist. Aborting now", file=sys.stderr)
            sys.exit (-1)
        ...
        if self.debug and self.seed is None:
            print("debugging: using seed = 1 for US", file=sys.stderr)
    ...
    def print_info (self, fptr = sys.stdout):
        ...
        if isinstance (fptr, file):
            ...
            try:
                ...
                print("could not write info to file ", fptr, file=sys.stderr)
        else:
            print("Should not get here... Hoping for the best" , file=sys.stderr)

    def run_test (self):
        # run a test run to see if the program behaves as expected
        if self.debug:
            print("US: running test simulation...", file=sys.stderr)
        ...
        if ret != 0:
            print("test failed. Aborting", file=sys.stderr)
            sys.exit (-1)
    ...
    def run (self, nsteps = 1):
        ...
        try:
            ...
            print("Could not open input file", self.input, "in directory", self.dir, "Aborting now!", file=sys.stderr)
            os.chdir ('..')
            return 1
        ...
        try:
            ...
            print("Could not copy", conf_file, "in", stored, file=sys.stderr)
            os.chdir ('..')
            return 1
        ...
        for it in range (1, nsteps + 1):
            ...
            if myinput.wait () != 0:
                print("execute program died unexpectedely. Not following it all the way down to hell, but expect more of these messages...", file=sys.stderr)
                os.chdir  ('..')
                break
