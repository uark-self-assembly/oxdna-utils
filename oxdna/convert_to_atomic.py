...
if len(sys.argv) < 4:
    print("Usage is %s dd_file configuration topology" % sys.argv[0], file=sys.stderr)
    sys.exit(1)
...
if np.any(box_angstrom[box_angstrom > 999]):
    print("At least one of the box sizes is larger than 999: all the atoms which are outside of the box will be brought back", file=sys.stderr)
...
for nucleotide in s._nucleotides:
    ...
    if my_base.chain_id != old_chain_id:
        if old_chain_id != -1:
            print("TERRING: ", old_chain_id, my_base.chain_id, file=sys.stderr)
            print("TER")
...
def print_P_distance_dd(nucls, divide_by=1):
    if len(nucls) != 24:
        print("%s can be used for dodecamers only" % print_P_distance_dd.__name__, file=sys.stderr)
        sys.exit(1)
